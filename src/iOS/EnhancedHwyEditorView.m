...
#import "EnhancedHighwayEditor.h"
#import "OsmObjects.h"
#import "VectorMath.h"
...
@implementation EnhancedHwyEditorView
...
-(void)rotateButtonForDirection
{
    if ( self.restriction == TURN_RESTRICT_NONE ) {
        CGFloat angle = [EnhancedHwyEditorView headingFromPoint:_centerPoint toPoint:_endPoint];
...
-(void)createOneWayArrowAtPosition:(CGPoint)location isDirection:(BOOL)isForward
{
    ...
    CGFloat angle = isForward ? [EnhancedHwyEditorView headingFromPoint:location toPoint:self.center] : [EnhancedHwyEditorView headingFromPoint:self.center toPoint:location];
